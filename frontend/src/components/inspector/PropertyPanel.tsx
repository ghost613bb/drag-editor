...
function renderTypeSpecificFields(node: ComponentNode) {
  switch (node.type) {
    case 'banner':
      return (
    ...
    case 'text':
      return (
        <div className="property-group">
          ...
          <div className="property-list">
            <PropertyRow label="文本内容" value={node.props.content} />
...
export function PropertyPanel({ pageTitle, selectedId, selectedNode }: PropertyPanelProps) {
  if (!selectedNode) {
    return (
  ...
  return (
    <div className="property-panel-body">
      ...
      {renderTypeSpecificFields(selectedNode)}
    </div>
  )
}
